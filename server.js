...
io.sockets.on('connection', function (socket) {
	...
	users.get(111954, function (err, result) {
		socket.user = result;
		logger.log('verbose', 'new Connection', {user: socket.user._id});
	...
	socket.on('change status', function (data) {
		if (!users.hasPermission(socket.user, 'ticket_status_change'))
			return;
...
logger.log('info', 'Server is running at http://localhost:' + conf.port + '/');
